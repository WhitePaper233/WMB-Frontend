<template>
    <div class="max-w-screen-xl mx-auto bg-white p-8 rounded-lg shadow-2xl">
        ...
        <form action="#" method="POST" @submit="openVerification">
            ...
            <div>
                <label for="content" class="block text-sm font-medium text-gray-700">留言 ( {{ content.length }} / 1000 字 ) </label>
                <div class="mt-1">
                    <textarea 
                    @input="checkContentLength"
                    @click="resetContentStyle"
                    id="content" 
                    name="content" 
                    rows="5" 
                    v-model="content"
            ...
            <fieldset>
                <button type="submit"
                    ...
                    <svg
                    v-if="isSubmitted"
                    class="animate-spin h-4 w-4 mr-3 text-white"
                    viewBox="0 0 24 24"
                    >
                    ...
                    {{ isSubmitted ? "正在进行人机验证" : "留言" }}
                </button>
                    
                <button
                @click="clearForm"
                type="reset" 
                class="ml-1 inline-flex justify-center items-center rounded-md border border-transparent bg-white py-2 px-4 text-base font-medium hover:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-gray-200 focus:ring-offset-1"
                >清空
                </button>
            </fieldset>
        </form>
...
let emit = defineEmits(["postComment"]);
